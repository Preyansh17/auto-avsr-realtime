...
class UltraFastBatchInferencePipeline:
    ...
    def _load_vocab_constraint(self):
        ...
        with open(self.vocab_file, 'r') as f:
            for line in f:
                line = line.strip()
                if line:
                    # 处理句子：strip().upper().replace(" ", "▁")
                    processed_sentence = line.upper().replace(" ", "▁")
                    sentences.append(processed_sentence)
        
        print(f"\nLoaded {len(sentences)} sentences for vocabulary constraint")
        
        # 初始化text transform来获取tokenizer
        ...
        print(f"\n{'='*70}")
        print(f"Sentence → Token IDs Mapping:")
        print(f"{'='*70}")
        ...
        for i, sentence in enumerate(sentences):
            ...
            sentence_to_tokens[sentence] = token_ids_list
            
            # 打印映射关系（只显示前35个句子避免输出过长）
            if i < 35:
                print(f"{sentence:40s} → {token_ids_list}")
            elif i == 35:
                print(f"... (showing first 35 sentences, total {len(sentences)} sentences)")
        
        # 重要：添加 blank token (0)，CTC 需要它
        ...
        print(f"\nAllowed token IDs (sorted): {self.allowed_token_ids}")
        
        # 显示一些 token 的文本表示
        print(f"\nSample token texts (first 20):")
        for i, token_id in enumerate(self.allowed_token_ids[:20]):
            if token_id < len(text_transform.token_list):
                token_text = text_transform.token_list[token_id]
                print(f"  {token_id:4d}: '{token_text}'")
    ...
    def process_batch_ultra_fast(self, video_paths, output_file=None, test_mode=False):
        ...
        run_name = f"video_inference_{int(time.time())}"
        wandb.init(
            project="limited_vocab",
            name=run_name,
            config={
                ...
                "video_dirs": getattr(self.cfg, "video_dirs", "video_50_25p,video_300_25p"),
        ...
        for i, video_path in enumerate(pbar):
            ...
            if result["status"] == "success":
                ...
                if token_ids:
                    ...
                    from preparation.transforms import TextTransform
                    text_transform = TextTransform()
                    token_texts = [text_transform.token_list[token_id] for token_id in token_ids if token_id < len(text_transform.token_list)]
                    print(f"✓ [{successful:3d}] {filename}: {result['transcript']} (WER: {wer:.3f})", flush=True)
                    print(f"   Token IDs: {token_ids}", flush=True)
                    print(f"   Tokens: {token_texts}", flush=True)
                else:
                    print(f"✓ [{successful:3d}] {filename}: {result['transcript']} (WER: {wer:.3f})", flush=True)
                pbar.set_postfix({
                    "Success": successful,
...
def main(cfg: DictConfig):
    ...
    detectors_str = env_detectors or cfg_detectors or "retinaface"
    ...
    # 词汇限制参数（可选）
    vocab_file = getattr(cfg, "vocab_file", None) or os.environ.get("VOCAB_FILE") or None
    if vocab_file == "":
        vocab_file = None
    # 默认值
    if vocab_file is None and hasattr(cfg, "use_vocab_constraint") and cfg.use_vocab_constraint:
        vocab_file = "/home/th3482/auto-avsr/vocab.txt"
    
    if vocab_file:
        print(f"Using vocabulary constraint from: {vocab_file}")
    else:
        print("No vocabulary constraint (all tokens allowed)")
