...
class UltraFastAudioBatchInferencePipeline:
    def __init__(self, cfg):
        self.cfg = cfg
        self.modality = "audio"
        self.device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        print(f"Using device: {self.device} | Modality: audio")
        self._init_model()
    ...
    def process_batch(self, audio_paths, output_prefix, test_mode=False):
        ...
        print(f"Processing {len(audio_paths)} audio files (Audio-only)...")
...
def main(cfg: DictConfig):
    # Get audio directories from config or env
    env_dirs = os.environ.get("AUDIO_DIRS")
    cfg_dirs = getattr(cfg, "audio_dirs", None)
    ...
    audio_dirs = [d.strip() for d in str(dirs_str).split(",") if d.strip()]
    ...
    for audio_dir in audio_dirs:
        ...
        audio_paths.sort()
        print(f"Dir={audio_dir} Found {len(audio_paths)} audio files")
        if len(audio_paths) == 0:
            continue

        pipeline = UltraFastAudioBatchInferencePipeline(cfg)

        dir_tag = os.path.basename(audio_dir.rstrip('/'))
        ts = int(time.time())
        base = f"ultra_fast_audio_{dir_tag}_{ts}"
        ...
        print(f"Completed Audio Dir={audio_dir}. Outputs: {os.path.abspath(prefix)}.json/.jsonl")
