...
class ModelModule(LightningModule):
    def __init__(self, cfg):
        ...
        if self.lora_enabled:
            ...
            scopes = getattr(self.cfg.lora, "scopes", ["encoder", "aux_encoder", "decoder"])
            ...
            print(f"\n✓ LoRA enabled: replaced {replaced} Linear layers; training only LoRA params")
        ...
        if self.freeze_enabled and not self.lora_enabled:
            ...
            print(f"\n✓ Freeze enabled: froze {frozen_count} parameters; training remaining params")
    ...
    def _load_vocab_constraint(self, vocab_file):
        ...
        # Add blank token (0) for CTC
        allowed_tokens.add(0)
        return sorted(list(allowed_tokens))

    def configure_optimizers(self):
        params = [p for p in self.model.parameters() if p.requires_grad]
        optimizer = torch.optim.AdamW([{"name": "lora", "params": params, "lr": self.cfg.optimizer.lr}], weight_decay=self.cfg.optimizer.weight_decay, betas=(0.9, 0.98))
    ...
    def forward(self, video, audio, allowed_token_ids=None, return_tokens=False):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        try:
            ...
            if dataset_ids is not None:
                # dataset_ids comes from collate_pad, shape is typically [B, 1, 1]
                dataset_ids = dataset_ids.view(-1).tolist()
            else:
                dataset_ids = [0] * len(videos)
            for i in range(len(videos)):
                video_input = videos[i:i+1]
                audio_input = audios[i:i+1]
                sample_target = targets[i]
                dataset_id = int(dataset_ids[i]) if i < len(dataset_ids) else 0

                predicted = self.forward(
                    video_input.squeeze(0),
    ...
    def on_validation_epoch_end(self):
        if self.val_total_length > 0:
            ...
            # Split reporting for merged validation set:
            # - legal298 samples
            # - legacy patient unseen samples
            ...
            if len(self.val_predictions) > 0 and hasattr(self.logger, 'experiment'):
                ...
                self.logger.experiment.log({f"validation_predictions_epoch_{self.current_epoch}": table})
