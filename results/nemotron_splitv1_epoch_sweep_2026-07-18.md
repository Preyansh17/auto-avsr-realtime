...
**Caveat on `_patient_data.sh`:** the sbatch's `merged` branch unconditionally overwrites `ROOT_DIR` back to its own `MERGED_ROOT` default even when a caller passes `ROOT_DIR=` via `--export` (unlike `TRAIN_FILE`/`VAL_FILE`/`TEST_FILE`, which the script does respect as overrides). This didn't break anything here only because `MERGED_ROOT`'s own symlinks happen to point at the exact same source directories as split_v1's — confirmed by inspection before trusting the first training run's manifests. **Fixed 2026-07-24:** the script now captures a caller-supplied `ROOT_DIR` before the `case` block and reapplies it afterwards, logging `[_patient_data] ROOT_DIR overridden by caller: <old> -> <new>` so it is never silent. Applied before the `PATIENT_DATA_ROOT` block so that mechanism keeps final precedence and its existing semantics are unchanged. Verified across four cases: no override (works under `set -u`), override applied, override identical to the default (no spurious log), and empty-string override (correctly ignored).
...
**Caveat:** `SaveBestWER.best_wer` resets to `inf` on resume (it's a plain Python attribute, not persisted via Lightning's callback state hooks) — the first post-resume validation epoch unconditionally overwrites `nemotron_best.nemo` even if it's worse than the pre-resume best. Mitigated during the investigation by backing up `nemotron_best.nemo` → `nemotron_best_salvage_pre_resume.nemo` before every resume. **Fixed 2026-07-24:** `SaveBestWER` now implements Lightning's `state_dict`/`load_state_dict` callback-state hooks, so `best_wer` rides along in the `.ckpt` and is restored on resume (printing `[resume] SaveBestWER restored best_wer=...`). The manual pre-resume backup is no longer necessary.
...
- ~~LoRA restore bug~~ **fixed 2026-07-25** (`asr_baselines/nemotron_restore.py`, used by both eval scripts). Diagnosis turned out to be two-layered: through the abstract base, `ASRModel.restore_from()` raises `TypeError: Can't instantiate abstract class ASRModel...` — NeMo failing to dispatch to the concrete subclass, which *masks* the real fault; through the concrete class, `EncDecRNNTBPEModel.restore_from()` surfaces the originally-reported `AttributeError: 'ConformerEncoder' object has no attribute 'add_adapter'`. Root cause: a saved adapter checkpoint carries an `adapters` config section, so `__init__` runs `setup_adapters()` while the encoder is still a plain `ConformerEncoder`, before anything calls `replace_adapter_compatible_modules()` — and that call cannot happen before `__init__` finishes. Workaround restores with the `adapters` section stripped, makes the encoder adapter-compatible, re-declares the adapters, then loads their weights from the checkpoint's own state dict. Verified end-to-end through both `nemotron_eval.py` and `nemotron_streaming_eval.py`: 96 adapter tensors restored with `missing=0 unexpected=0` and `enabled=['lora']`, and a control full-FT checkpoint still evaluates to its expected 27.50% (no regression on the common path). Note the smoketest LoRA checkpoint itself scores 87.5% WER — it is a few-epoch code-path smoketest, never a trained model; this fixes loading, not LoRA's quality, which was never competitive with full-FT here.
