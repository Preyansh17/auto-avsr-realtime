...
## Whisper medium: the tune transfers, but the model doesn't -- large-v3 dominates outright (2026-07-19)

large-v3 is heavy for real-time deployment, so the obvious question is whether the confirmed recipe works on a cheaper base. Trained `openai/whisper-medium` (764M params vs large-v3's ~1.55B) with the identical tune (warmup=25 / wd=0.01 / lr=1e-5 / 30 epochs), merged, seed1, and decoded at both operating points -- the optimum was not assumed to carry over. (`MODEL` is now an env var on `slurm/whisper_wdwarmup_finetune.sbatch`; the bit-exact converter, used here on a non-large-v3 model for the first time, passed unchanged.)

| Model | Decode | Offline WER | Streaming WER | RTF | TTFT p50 | word-lag p50 |
| --- | --- | --- | --- | --- | --- | --- |
| **large-v3** | **1.2s / ft=18** | **7.62%** | **8.97%** | **0.170** | 2.01s | 1.81s |
| large-v3 | 0.6s / ft=25 | 7.62% | 11.66% | 0.300 | 1.51s | 1.60s |
| medium | 1.2s / ft=18 | 12.11% | 18.83% | 0.122 | 1.89s | 1.67s |
| medium | 0.6s / ft=25 | 12.11% | **16.59%** | 0.175 | 1.49s | 1.45s |

**Medium is strictly dominated, so there is no deployment argument for it.** Compare each model at its own best decode point: medium manages 16.59% at RTF 0.175, while large-v3 manages **8.97% at RTF 0.170** -- less compute *and* half the word errors. The intuition that a smaller model must be cheaper to serve fails here, because the longer segment large-v3 prefers means far fewer forward passes over the growing buffer (190 chunks vs 361), which more than offsets its higher per-pass cost. **Model size and decode configuration are not independent choices, and comparing model sizes at a fixed decode point would have produced the wrong answer.**

That is the second useful thing here: **medium's optimal decode point is different from large-v3's.** Medium prefers 0.6s/ft=25 (16.59%) over 1.2s/ft=18 (18.83%) -- the reverse of large-v3's ordering. So "1.2s/ft=18 is best" is a property of *this checkpoint*, not of AlignAtt or of the data, and any future base-model change should re-sweep decode rather than inherit the number (exactly the mistake that left ~3pp on the table when the recipe changed earlier in this file).

The tune itself does appear to transfer -- medium's numbers are respectable in absolute terms, it simply starts from a worse place -- but that was not isolated (no untuned-medium control was run), so it is an impression, not a measurement.

**Tension worth recording:** this contradicts the CarelessWhisper line's finding, where bigger was distinctly *not* better on this same ~300-clip budget (val->test gap grew monotonically with capacity: small 14.1pp, large-v2 20.9pp, large-v3 31.3pp, and `small` won outright). Here the largest model wins by a wide margin. Plausible reasons the two disagree: different codebase and recipe (LoRA-first there vs full-FT here), a different streaming implementation, and CarelessWhisper's comparison sat at a far worse absolute WER (34%+) where other failure modes likely dominate the capacity effect. Not resolved -- just flagged, since "bigger is not better on this dataset" should no longer be treated as an established fact of the project without qualifying which pipeline it referred to.
