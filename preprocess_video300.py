#!/usr/bin/env python3
"""
Preprocess /scratch/th3482/LipVideoData/video_300_25p short clips into mouth-crop datasets
for both retinaface and mediapipe detectors, splitting randomly into train/val/test (200/50/50).
...
def process_one(path: str, split: str, detector: str, out_root: str, text_transform: TextTransform, combine_av: bool, convert_gray: bool = False) -> Tuple[bool, str, int, str, str, str]:
...
def main():
    parser = argparse.ArgumentParser(description="Preprocess video_300_25p into cropped datasets for two detectors")
    parser.add_argument("--input_dir", type=str, default="/scratch/th3482/LipVideoData/video_300_25p")
    parser.add_argument("--output_root", type=str, default="/scratch/th3482/LipVideoData/video_300_25p_crops")
    ...
    for detector in detectors:
        ...
        for split, files in split_files.items():
            for i, path in enumerate(tqdm(files, desc=f"{detector}-{split}")):
                success, split, nframes, token_ids, ds_name, rel_base = process_one(
                    path, split, detector, out_root, text_transform, args.combine_av, convert_gray=args.convert_gray
                )
                if success:
                    ok += 1
