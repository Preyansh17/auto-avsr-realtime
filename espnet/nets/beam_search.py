...
class BeamSearch(torch.nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            allowed_token_ids (list[int]): List of allowed token IDs for constrained decoding.
                If None, all tokens are allowed (no constraint).
        ...
        # Setup vocabulary constraint mask
        self.vocab_mask = None
        self.allowed_token_ids = None
        self.n_allowed_tokens = vocab_size  # default: all tokens allowed
        if allowed_token_ids is not None:
            # Create mask: -inf for disallowed tokens, 0 for allowed tokens
            self.vocab_mask = torch.full((vocab_size,), float('-inf'))
            self.allowed_token_ids = allowed_token_ids
            self.vocab_mask[allowed_token_ids] = 0.0
            # Always allow SOS and EOS tokens
            self.vocab_mask[sos] = 0.0
            self.vocab_mask[eos] = 0.0
            # Count number of allowed tokens (including SOS and EOS)
            self.n_allowed_tokens = len(set(allowed_token_ids).union({sos, eos}))
