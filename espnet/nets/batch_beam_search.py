...
class BatchBeamSearch(BeamSearch):
    ...
    def search(self, running_hyps: BatchHypothesis, x: torch.Tensor) -> BatchHypothesis:
        ...
        # Apply vocabulary constraint mask BEFORE pre-beam (if specified)
        # This ensures pre-beam only considers allowed tokens
        
        # partial scoring
        if self.vocab_mask is not None:
            # If we have vocab constraint, always use allowed tokens
            # Include SOS and EOS tokens with allowed tokens
            allowed_tokens = set(self.allowed_token_ids).union({self.sos, self.eos})
            allowed_tokens_list = sorted(list(allowed_tokens))
            part_ids = torch.tensor(allowed_tokens_list, device=x.device).unsqueeze(0).expand(n_batch, -1)
        elif self.do_pre_beam:
            # No vocab constraint, use pre-beam
            pre_beam_scores = (
                weighted_scores
        ...
        # Re-apply vocabulary constraint mask AFTER all scoring
        # This ensures no non-allowed tokens can be selected
        if self.vocab_mask is not None:
            weighted_scores += self.vocab_mask.to(device=x.device, dtype=x.dtype)
