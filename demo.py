...
class InferencePipeline(torch.nn.Module):
    def __init__(self, cfg, detector="retinaface"):
        ...
        # 加载模型到指定设备
        ckpt = torch.load(cfg.pretrained_model_path, map_location=self.device)
    ...
    def forward(self, data_filename):
        ...
        if self.modality == "video":
            with torch.no_grad():
                transcript = self.modelmodule(video.to(self.device))
        elif self.modality == "audio":
            with torch.no_grad():
                transcript = self.modelmodule(audio.to(self.device))
            ...
            with torch.no_grad():
                transcript = self.modelmodule(video.to(self.device), audio.to(self.device))
